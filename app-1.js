...
function createUserDiv(obj){
    ...
    div.className = 'card'
    createActualNameHeader(obj)
}
function createActualNameHeader(actualName){
    const name = createElement('h1')
    name.id = actualName.id
    name.className = 'name'
    name.textContent = actualName.name
    div.appendChild(name)
    name.addEventListener('click', getPosts)  //see getPosts function below
    createUserNameSubheader(userName)
}

function createUserNameSubheader(uName){
    const userName = document.createElement('h2')
    userName.textContent = uName.userName
    div.appendChild(userName)
    cityNameInParagraph(cityName)
}

function cityNameInParagraph(loc){
    const city = document.createElement('p')
    city.textContent = loc.address.city
    div.appendChild(city)
    appendDivToContainer(div)
}
 
function getPosts(e){
    $.get(`https://jsonplaceholder.typicode.com/${e.target.id}/posts`, logPosts)
}
